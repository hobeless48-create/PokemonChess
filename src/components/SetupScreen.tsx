...
export const SetupScreen: React.FC<SetupScreenProps> = ({
  ...
  const getTypeCounts = (slots: string[]) => {
    ...
    slots.forEach(sp => {
      ...
      if (!db) return;
      const types = [db.t1];
      if (db.t2 && db.t2 !== "None") types.push(db.t2);
  ...
  React.useEffect(() => {
    if (p2pStatus === "connected") {
      if (peerP1Slots) setP1Slots(peerP1Slots);
      if (peerP1Placements) setP1Placements(peerP1Placements);
      if (peerP2Slots) setP2Slots(peerP2Slots);
      if (peerP2Placements) setP2Placements(peerP2Placements);
    }
  }, [peerP1Slots, peerP1Placements, peerP2Slots, peerP2Placements, p2pStatus]);
    ...
    <div id="screen-setup" className="max-w-4xl mx-auto p-4 md:p-6 bg-[#16213e]/90 rounded-2xl shadow-2xl border border-[#0f3460]">
            ...
            <div className="filter-tabs flex gap-2">
              {["all", "legendary", "mythical", "non-legendary"].map(rv => {
                ...
                const selectedStyles: Record<string, string> = {
                  legendary: "bg-[#ffd700] text-[#1a1a2e] border-[#ffd700] font-bold",
                  mythical: "bg-purple-600 text-white border-purple-600 font-bold",
                  "non-legendary": "bg-slate-500 text-white border-slate-500 font-bold",
                  all: "bg-[#4fc3f7] text-[#1a1a2e] border-[#4fc3f7] font-bold",
                };
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div className="team-section bg-[#0f0f1a] rounded-xl p-4 border border-[#4fc3f7]/30">
          ...
          {(() => {
            const p1TypeCounts = getTypeCounts(p1Slots);
        ...
        <div className="team-section bg-[#0f0f1a] rounded-xl p-4 border border-[#ef5350]/30">
          ...
          {(() => {
            const p2TypeCounts = getTypeCounts(p2Slots);
